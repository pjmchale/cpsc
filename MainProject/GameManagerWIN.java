...
public class GameManagerWIN{
  ...
  GameManagerWIN(){
    ...
    players[1] = new Player("Matt");

    initializeMap();
    initializeTurn();

    allCountries = map.getCountries();
    numCountries = allCountries.size()-1;
    for(int i=0;i < numCountries; i++){
      country = allCountries.get(i);
      //System.out.println(country.getName());
      country.setOwner(currentPlayer);
      currentPlayer.gainCountry(country);
      country.setUnits(15);
    }

    nextTurn();
    country = allCountries.get(numCountries);
    country.setOwner(currentPlayer);
    currentPlayer.gainCountry(country);
    country.setUnits(1);
